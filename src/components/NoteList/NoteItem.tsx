...
export function NoteItem({
  ...
  const [isHovered, setIsHovered] = useState(false); // ノートアイテムがホバーされているかどうか
  const getIcon = () => {
    return expanded ? ChevronDown : isHovered ? ChevronRight : FileIcon; // ホバーされているときはChevronRight、そうでなければFileIcon
  };

  const menu = (
    // ドロップダウンメニュー, isHoveredがtrueのときは表示される
